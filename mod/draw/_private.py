...
def private(prompt, to):
    try:
        ...
        if "cogview" in (set.setdir["draw_model"]) or "stabilityai/" in (set.setdir["draw_model"]):
            data = {
                "model": set.setdir["draw_model"],
                "prompt": prompt,
            }
        else:
            data = {
        ...
        send.msg({'msg_type': 'private', 'number': to, 'msg': '正在绘画[%s]中...' % prompt})
        response = requests.post(url=urldraw, headers=headers, stream=True, data=json.dumps(data))
        if response.status_code == 200:
            send.msg({'msg_type': 'private', 'number': to, 'msg': '绘画完毕发送中...'})
        ...
        for n in range(0, max_n):
            try:
                ...
                send.image({'msg_type': 'private', 'number': to, 'msg': name})
                break
            except:
                print(n)
                if n == max_n-1:
        ...
        print('绘画错误:', e)
        send.msg({'msg_type': 'private', 'number': to, 'msg': 'AI绘画操作无法执行'})
